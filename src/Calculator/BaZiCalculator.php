...
class BaZiCalculator
{
     ...
     * @param bool $mainQiOnly 是否仅返回主气十神（默认true）
     *
     * @return string 十神名称（如“劫财”、“正财”、“伤官”等），若无法识别返回空字符串，若存在多个则用英文逗号隔开
     */
    public static function getShiShen(string $dayGan, string $target, bool $mainQiOnly = true): string
    {
        $targetGan = [];
        $result = [];
        ...
        if (count($targetGan)) {
            foreach ($targetGan as $_targetGan) {
                ...
                $he = BaZiConstants::TIANGAN_WUXING[$_targetGan];
                $meYinYang = BaZiConstants::TIANGAN_YINYANG[$dayGan];
                $heYinYang = BaZiConstants::TIANGAN_YINYANG[$_targetGan];
                if ($me === $he) {
                    $key = '同我-' . ($meYinYang === $heYinYang ? '同性' : '异性');
                    $result[] = BaZiConstants::SHI_SHEN_MAP[$key] ?? '';
                    continue;
                }
    ...
    public static function getYearPillar(string|\DateTimeInterface $date): array
    {
        ...
        return [
            'tiangan' => BaZiConstants::TIANGAN[$offset % 10],
            'dizhi' => BaZiConstants::DIZHI[$offset % 12]
        ];
    }
    ...
    public static function getMonthPillar(string|\DateTimeInterface $date): array
    {
        ...
        $startIndex = array_search($startGan, BaZiConstants::TIANGAN);
        // 月干是从正月开始依次排列的
        $ganIndex = ($startIndex + $monthIndex) % 10;
        $monthGan = BaZiConstants::TIANGAN[$ganIndex];
    ...
    public static function getDayPillar(string|\DateTimeInterface $date): array
    {
        ...
        $offset = ($hour == 23) ? 1 : 0;
        return [
            'tiangan' => BaZiConstants::TIANGAN[($days + $offset) % 10],
            'dizhi' => BaZiConstants::DIZHI[($days + $offset) % 12]
        ];
    }
    ...
    private static function calcHourGan(string $dayGan, string $dizhi): string
    {
        ...
        // 计算时干
        $startIdx = array_search($startGan, BaZiConstants::TIANGAN);
        $dizhiIdx = array_search($dizhi, BaZiConstants::DIZHI);
        return BaZiConstants::TIANGAN[($startIdx + $dizhiIdx) % 10];
    }
}
